...
# Load the data
setwd("E:/intershala/1452762979_586__HR_Employee_Attrition_Data.csv.zip_attachment_658891")
HR_emp_att<-read.csv("HR_Employee_Attrition_Data.csv",header = T,sep = ",",na.strings = c("",NA))
...
# Observations: 2,940 with Variables: 35
 
# Class Label is Attrition with 2466 'NO' and 474 'Yes' that shows the unbalance class label. we have to pay attention to the unbalance class algorithm problems!
...
g3 <- ggplot(HR_emp_att, aes(x = HourlyRate, fill = Attrition)) + geom_density(alpha = 0.7) + scale_fill_manual(values = c("#386cb0","#fdb462"))

g4 <- ggplot(HR_emp_att, aes(x = DailyRate, fill = Attrition)) + geom_density(alpha = 0.7) + scale_fill_manual(values = c("#386cb0","#fdb462"))

g5 <- ggplot(HR_emp_att, aes(x = MonthlyRate, fill = Attrition)) + geom_density(alpha = 0.7) + scale_fill_manual(values = c("#386cb0","#fdb462"))
...
attrition_edufield <- HR_emp_att %>%select(Attrition, EducationField) %>%group_by(Attrition, EducationField) %>% summarise(count = n())
# creating datatable
...
col="red",aes(fill=..count..))+ labs(x="Age", y="Count")+ scale_fill_gradient("Count", low="green", high="red")  
...
HR_emp_att$AgeGroup <- as.factor(ifelse(HR_emp_att$Age<=24,"Young", ifelse(HR_emp_att$Age<=54,"Middle-Age","Adult")))
